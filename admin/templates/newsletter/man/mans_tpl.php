...
<section class="content">
    ...
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            ...
            <?php if(isset($config['newsletter'][$type]['is_send']) && $config['newsletter'][$type]['is_send'] == true) { ?>
            <p class="d-block text-secondary w-100 float-left mb-0 mt-1">Chọn email sau đó kéo xuống dưới cùng danh sách
                này để có thể thiết lập nội dung email muốn gửi đi.</p>
            <?php } ?>
        </div>
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php if(isset($config['newsletter'][$type]['show_datetime']) && $config['newsletter'][$type]['show_datetime'] == true) { ?>
                        <th class="align-middle">Ngày đặt bàn</th>
                        <?php } ?>
                    ...
                    <tr>
                        ...
                        <td class="align-middle">
                            <input type="number" class="form-control form-control-mini m-auto update-numb" min="0"
                                value="<?=$items[$i]['numb']?>" data-id="<?=$items[$i]['id']?>" data-table="newsletter">
                        </td>
                        <?php if(isset($config['newsletter'][$type]['show_name']) && $config['newsletter'][$type]['show_name'] == true) { ?>
                        <td class="align-middle">
                            <a class="text-dark text-break" href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>"
                        ...
                        <td class="align-middle">
                            <a class="text-dark text-break" href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>"
                                title="<?=$items[$i]['email']?>"><?=$items[$i]['email']?></a>
                        </td>
                        <?php } ?>
                        <?php if(isset($config['newsletter'][$type]['show_phone']) && $config['newsletter'][$type]['show_phone'] == true) { ?>
                        ...
                        <?php } ?>
                        <?php if(isset($config['newsletter'][$type]['show_datetime']) && $config['newsletter'][$type]['show_datetime'] == true) { ?>
                        <td class="align-middle"><?=date("h:i:s A - d/m/Y", $items[$i]['datetime'])?></td>
                        <?php } ?>
                        <?php if(isset($config['newsletter'][$type]['file']) && $config['newsletter'][$type]['file'] == true) { ?>
                        <td class="align-middle">
                            ...
                            <a class="btn btn-sm bg-gradient-primary text-white d-inline-block p-1 rounded"
                                href="<?=UPLOAD_FILE.$items[$i]['file_attach']?>" title="Download tập tin"><i
                        ...
                        <?php } ?>
                        <?php if(isset($config['newsletter'][$type]['confirm_status']) && count($config['newsletter'][$type]['confirm_status']) > 0) { ?>
                        <td class="align-middle"><?=$func->getStatusNewsletter($items[$i]['confirm_status'],$type)?>
                        </td>
                        <?php } ?>
                        <td class="align-middle text-center text-md text-nowrap">
                            <a class="text-primary mr-2" href="<?=$linkEdit?>&id=<?=$items[$i]['id']?>"
                                title="Chỉnh sửa"><i class="fas fa-edit"></i></a>
                            <a class="text-danger" id="delete-item" data-url="<?=$linkDelete?>&id=<?=$items[$i]['id']?>"
        ...
        <form name="frmsendemail" method="post" action="<?=$linkMan?>" enctype="multipart/form-data">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <strong
                        class="d-block mt-2 mb-2 text-sm"><?php echo $config['newsletter'][$type]['file_type'] ?></strong>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control form-control-ckeditor" name="content" id="content" rows="5"
